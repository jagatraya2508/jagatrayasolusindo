...
function APAdjustmentList({ adjustmentType = 'DEBIT' }) {
    ...
    const checkApprovalPermission = async () => {
        try {
            const txType = adjustmentType === 'DEBIT' ? 'ap-debit-adjustment' : 'ap-credit-adjustment';
    ...
    const fetchMasterData = async () => {
        try {
            ...
            if (transData.success) {
                // Filter transcodes for AP adjustment (nomortranscode = 30 for CREDIT/IN, 31 for DEBIT/OUT)
    ...
    const fetchInvoices = async (partnerId) => {
        try {
            const response = await fetch(`/api/ap-invoices/for-allocation?partner_id=${partnerId}`);
            const data = await response.json();
            if (data.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editingItem ? `/api/ap-adjustments/${editingItem}` : '/api/ap-adjustments';
    ...
    const handleEdit = async (id) => {
        try {
            const response = await fetch(`/api/ap-adjustments/${id}`);
            const data = await response.json();
            if (data.success) {
                const adj = data.data;
                setFormData({
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`/api/ap-adjustments/${id}`, { method: 'DELETE' });
            const data = await response.json();
            if (data.success) {
                alert(data.message);
                fetchData();
    ...
    const handleApprove = async (id) => {
        ...
        try {
            const response = await fetch(`/api/ap-adjustments/${id}/approve`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await response.json();
    ...
    const handlePost = async (id) => {
        ...
        try {
            const response = await fetch(`/api/ap-adjustments/${id}/post`, { method: 'PUT' });
            const data = await response.json();
            if (data.success) {
                alert(data.message);
                fetchData();
    ...
    const handleUnapprove = async (id) => {
        ...
        try {
            const response = await fetch(`/api/ap-adjustments/${id}/unapprove`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await response.json();
    ...
    const handleUnpost = async (id) => {
        ...
        try {
            const response = await fetch(`/api/ap-adjustments/${id}/unpost`, { method: 'PUT' });
            const data = await response.json();
            if (data.success) {
                alert(data.message);
                fetchData();
    ...
    const handleAllocationChange = (invoiceId, amount) => {
        setFormData(prev => {
            const existing = prev.allocations.find(a => a.ap_invoice_id === invoiceId);
            if (existing) {
                return {
                    ...prev,
                    allocations: prev.allocations.map(a =>
                        a.ap_invoice_id === invoiceId ? { ...a, allocated_amount: parseFloat(amount) || 0 } : a
                    )
                };
            } else {
                return {
                    ...prev,
                    allocations: [...prev.allocations, { ap_invoice_id: invoiceId, allocated_amount: parseFloat(amount) || 0 }]
                };
            }
        });
    };
                                        ...
                                        <table className="data-table">
                                                ...
                                                {invoices.length === 0 ? (
                                                    <tr>
                                                        <td colSpan="5" style={{ textAlign: 'center', padding: '1rem', color: '#888' }}>
                                                            Tidak ada invoice outstanding untuk supplier ini.
                                                        </td>
                                                    </tr>
                                                ) : (
                                                    invoices.map(inv => {
                                                        const allocation = formData.allocations.find(a => a.ap_invoice_id === inv.id);
                                                        const outstandingAmount = parseFloat(inv.outstanding_amount || 0);
                                                        return (
                                                            <tr key={inv.id}>
                                                                <td>{inv.doc_number}</td>
                                                                <td>{formatDate(inv.doc_date)}</td>
                                                                <td>{formatMoney(inv.total_amount || 0)}</td>
                                                                <td style={{ color: '#e74c3c', fontWeight: 'bold' }}>{formatMoney(outstandingAmount)}</td>
                                                                <td>
                                                                    <input
                                                                        type="number"
                                                                        min="0"
                                            ...
                                            <tfoot>
                                                <tr style={{ fontWeight: 'bold', backgroundColor: '#f5f5f5' }}>
                                                    <td colSpan="4" style={{ textAlign: 'right' }}>Total Alokasi:</td>
